...
class TrackArchive:
    ...
    def _initialize_db(self) -> None:
        """Creates the database and table with optimized settings and indexes if they don't exist."""
        try:
            with self._get_connection() as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_artist ON downloaded_tracks(artist);"
                )
                conn.commit()
        except sqlite3.Error as e:
    ...
    def _migrate_from_txt_if_needed(self, config_dir_path: Path) -> None:
        ...
        try:
            with open(txt_archive_path, "r", encoding="utf-8") as f:
            ...
            if track_ids:
                ...
                with self._get_connection() as conn:
                    ...
                    conn.commit()
                log.info(
                    f"[green]✓ Migrated {len(track_ids)} entries from the text archive.[/green]"
                )

            backup_path = txt_archive_path.with_suffix(".txt.migrated")
            os.rename(txt_archive_path, backup_path)
            log.info(
                f"[dim]The old text archive has been renamed to '{backup_path.name}'[/dim]"
            )
        ...
        try:
            with self._get_connection() as conn:
                for i in range(0, len(track_ids), BATCH_SIZE):
                    ...
                    query = f"SELECT track_id FROM downloaded_tracks WHERE track_id IN ({placeholders})"
                    cursor = conn.execute(query, chunk)
                    existing_ids = {row[0] for row in cursor.fetchall()}
                    results.update({tid: tid in existing_ids for tid in chunk})
            return results
        except sqlite3.Error as e:
            log.error(f"Batch archive check failed: {e}")
            return {tid: False for tid in track_ids}
        ...
        try:
            with self._get_connection() as conn:
                for i in range(0, len(records), BATCH_SIZE):
                    ...
                    conn.executemany(
                        "INSERT OR IGNORE INTO downloaded_tracks (track_id, artist, album, title) VALUES (?, ?, ?, ?)",
                        chunk,
                    )
                conn.commit()
            return True
    ...
    def _get_stats_sync(self) -> Optional[Dict[str, Any]]:
